...
class nsqphp
{
    ...
    public function mpublish($topic, array $msgs)
    {
        ...
        $nsqdCount = $this->pubConnectionPool->count();
        $msgsCount = count($msgs);

        if ($nsqdCount === 1) {
            return $this->mpublishProcessing($topic, $msgs, 0);
        }

        $chunks = array_chunk($msgs, ceil($msgsCount / $nsqdCount));

        foreach ($chunks as $index => $msgsChunk) {
            try {
                $this->mpublishProcessing($topic, $msgsChunk, $index);
            } catch (Exception\PublishException $e) {
                // in case of publishing error just trying to move the chunk to next nsqd if available
                if (isset($chunks[$index + 1])) {
                    /** @noinspection SlowArrayOperationsInLoopInspection */
                    $chunks[$index + 1] = array_merge($msgsChunk, $chunks[$index + 1]);
                } else {
                    throw $e;
                }
            }
    ...
    private function mpublishProcessing($topic, array $msgs, $connIndex = 0)
    {
        $errors = [];

        $conn = $this->pubConnectionPool[$connIndex];

        /** @var $conn ConnectionInterface */
        try {
            $this->tryFunc(function (ConnectionInterface $conn) use ($topic, $msgs, &$errors) {
            ...
            throw new Exception\PublishException(
                sprintf(
                    'Failed to publish batch of messages to %s; Errors were: %s',
                    (string)$conn,
                    implode(', ', array_merge($errors, [$e->getMessage()]))
                )
            );
        }
